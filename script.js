...
const css = document.getElementById("lgcode");
...
class Slider {
    ...
    constructor({ DOMselector, sliders }) {
        ...
        this.arcFractionLength = 1;                                // Arc fraction length
        ...
        this.handleStrokeThickness = 10;                             // Slider handle stroke thickness    
    ...
    draw() {
        ...
        color1.addEventListener('input', this.setGradient.bind(this), false);
        color2.addEventListener('input', this.setGradient.bind(this), false);
        color3.addEventListener('input', this.setGradient.bind(this), false);
    }
    ...
    redrawActiveSlider(rmc) {
        ...
        this.setGradient(currentAngle);
    }
    
    setGradient(currentAngle) {
        const currentSlider = this.sliders[0];
        ...
        let degree;
        if(currentValue) {
            degree = `${currentValue}deg`;
            degreeArray.push(currentValue);
        } else {
            if(degreeArray.length === 0) {
                degree = `${this.sliders[0].initialValue}deg`;
            } else {
                degree = `${degreeArray[degreeArray.length-1]}deg`;
            }
        };
        body.style.background = `linear-gradient(${degree}, ${color1.value}, ${color2.value}, ${color3.value})`;
    ...
    updateLegendUI(currentAngle) {
        ...
        // this.getCurrentValue(currentAngle);
